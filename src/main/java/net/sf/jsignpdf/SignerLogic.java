...
public class SignerLogic implements Runnable {
	...
	public boolean signFile() {
		...
		if (!(validateInOutFiles(options.getInFile(), outFile)
				|| validateInOutStream(options.getIsInFile(), options.getOsOutFile()))) {
		...
		try {
			...
			PdfReader reader;
			try {
				if (options.getIsInFile() == null)
					reader = new PdfReader(options.getInFile(), options.getPdfOwnerPwdStrX().getBytes());
				else
					reader = new PdfReader(options.getIsInFile(), options.getPdfOwnerPwdStrX().getBytes());
			} catch (Exception e) {
				try {
					if (options.getIsInFile() == null)
						reader = new PdfReader(options.getInFile(), new byte[0]);
					else
						reader = new PdfReader(options.getIsInFile(), new byte[0]);
				} catch (Exception e2) {
			...
			if (options.getOsOutFile() == null)
				fout = new FileOutputStream(outFile);
			else
				fout = options.getOsOutFile();
			...
			if (reader.getPdfVersion() < hashAlgorithm.getPdfVersion()) {
				...
				/*
				 * if (options.isAppendX()) { // if we are in append mode and version should be
				 * updated // then return false (not possible)
				 * LOGGER.info(RES.get("console.updateVersionNotPossibleInAppendMode")); return
				 * false; }
				 */
			...
			if (options.isAdvanced() && options.getPdfEncryption() != PDFEncryption.NONE) {
				...
				case CERTIFICATE:
					...
					if (!KeyStoreUtils.isEncryptionSupported(encCert)) {
						LOGGER.error(
								RES.get("console.pdfEncError.cantUseCertificate", encCert.getSubjectDN().getName()));
						return false;
					}
			...
			if (options.isVisible()) {
				...
				if (tmpImgPath != null || options.getImage() != null) {
					LOGGER.info(RES.get("console.createImage", tmpImgPath));
					Image img = null;
					if (tmpImgPath != null)
						img = Image.getInstance(tmpImgPath);
					else
						img = Image.getInstance(tmpImgPath);
				...
				if (tmpBgImgPath != null || options.getBgImage() != null) {
					LOGGER.info(RES.get("console.createImage", tmpBgImgPath));
					Image img = null;
					if (tmpBgImgPath != null)
						img = Image.getInstance(tmpBgImgPath);
					else
						img = Image.getInstance(options.getBgImage());
	...
	/**
	 * Validates if input and output stream are valid for signing.
	 * 
	 * @param inFile  input file
	 * @param outFile output file
	 * @return true if valid, false otherwise
	 */
	private boolean validateInOutStream(final InputStream inFile, final OutputStream outFile) {
		LOGGER.info(RES.get("console.validatingFiles"));
		try {
			if (inFile == null || inFile.available() == 0) {
				LOGGER.info("Empty input stream.");
				return false;
			}
			if (outFile == null) {
